        ...
        this.getDanmu = function () {
            $.get(that.options.urlToGetDanmu, function (data, status) {
                ...
                for(let i = 0; i < data.data.length; i++){
                    ...
                    console.log(dd);
                    $(that.id + ' .danmu-div').danmu("addDanmu", dd);
                }
                danmuFromSql = eval(data);
                for (var i = 0; i < danmuFromSql.length; i++) {
                    console.log("sql");
                    console.log(danmuFromSql[i]);
                    dd = {
                        text: danmuFromSql[i].text,
                        color: danmuFromSql[i].color,
                        size: danmuFromSql[i].size,
                        position: danmuFromSql[i].position,
                        time: danmuFromSql[i].time
                    };
                    console.log(dd);
                    $(that.id + ' .danmu-div').danmu("addDanmu", dd);
                }
            ...
            if (e.data.that.options.urlToPostDanmu){
                ...
                $.ajax({
                    ...
                    beforeSend: function (XMLHttpRequest) {
                        XMLHttpRequest.setRequestHeader("token", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoiMTA4NjMwY2UtNzNhMy00YzUwLTk0NWUtNjJhNmFjZjFlNjY2IiwiZXhwIjoxNjMwOTE3MzM0LCJpYXQiOjE2MzAzMTI1MzQsInVzZXJuYW1lIjoibWlvbHluIn0.YJgDaYhnr84bQV13xXyIuU7lMO-Y-q7_I3goOIXjc-k");
                    },
